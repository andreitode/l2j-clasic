 ...
 */
package org.classiclude.gameserver.network.clientpackets.appearance;

import org.classiclude.gameserver.data.xml.AppearanceItemData;
import org.classiclude.gameserver.enums.InventorySlot;
import org.classiclude.gameserver.enums.ItemLocation;
import org.classiclude.gameserver.model.actor.Player;
import org.classiclude.gameserver.model.actor.request.ShapeShiftingItemRequest;
import org.classiclude.gameserver.model.holders.AppearanceHolder;
...
import org.classiclude.gameserver.network.serverpackets.appearance.ExShapeShiftingResult;
	...
	protected void runImpl()
	{
		...
		switch (appearanceStone.getType())
		{
			case RESTORE:
			{
				...
				{
					player.sendMessage("You do not have enough inventory space to restore the appearance.");
					break;
				}
				
				int appearanceStoneId = targetItem.getVariables().getInt(ItemVariables.VISUAL_APPEARANCE_STONE_ID, 0);
				if (appearanceStoneId > 0)
				{
					if (player.getInventory().addItem("RestoreAppearance", appearanceStoneId, 1, player, this) != null)
					{
						targetItem.getVariables().remove(ItemVariables.VISUAL_APPEARANCE_STONE_ID);
						targetItem.getVariables().storeMe();
					}
					else
					{
						player.sendMessage("Failed to return appearance stone to inventory.");
					}
				}
				else
				{
					player.sendMessage("No appearance stone found to return.");
				}
