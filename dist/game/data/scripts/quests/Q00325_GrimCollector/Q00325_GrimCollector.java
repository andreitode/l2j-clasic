...
public class Q00325_GrimCollector extends Quest
{
	...
	public String onEvent(String event, Npc npc, Player player)
	{
		...
		switch (event)
		{
			...
			case "30434-09.htm":
			{
				final long skeletons = getQuestItemsCount(player, COMPLETE_SKELETON);
				if (skeletons > 0)
				{
	...
	private static long getNumberOfPieces(Player player)
	{
	...
	{
		final long count = getNumberOfPieces(player);
		if (count > 0)
		{
			long reward = (30 * getQuestItemsCount(player, ZOMBIE_HEAD)) + (20 * getQuestItemsCount(player, ZOMBIE_HEART)) + (20 * getQuestItemsCount(player, ZOMBIE_LIVER)) + (100 * getQuestItemsCount(player, SKULL)) + (40 * getQuestItemsCount(player, RIB_BONE)) + (14 * getQuestItemsCount(player, SPINE)) + (14 * getQuestItemsCount(player, ARM_BONE)) + (14 * getQuestItemsCount(player, THIGH_BONE)) + (341 * getQuestItemsCount(player, COMPLETE_SKELETON));
			if (count > 10)
			{
				reward += 1629;
			}
