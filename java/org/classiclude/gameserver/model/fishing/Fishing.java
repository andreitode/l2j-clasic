 ...
 */
package org.classiclude.gameserver.model.fishing;
...
import org.classiclude.gameserver.model.events.AbstractScript;
import org.classiclude.gameserver.network.SystemMessageId;
import org.classiclude.gameserver.network.serverpackets.ExFishingHpRegen;
import org.classiclude.gameserver.network.serverpackets.ExFishingStartCombat;
...
public class Fishing implements Runnable
{
	...
	public Fishing(Player fisher, Fish fish, boolean isNoob, boolean isUpperGrade, int lureId)
	{
		...
		_fisher.sendPacket(new ExSendUIEventCustom(ExSendUIEventCustom.INITIALIZE_FISH_STATUS, _time, _fishMaxHp, String.valueOf(_mode), "", "", "", "", ""));
	...
	public void changeHp(int hp, int pen)
	{
		...
		_fisher.broadcastPacket(new ExSendUIEventCustom(ExSendUIEventCustom.UPDATE_FISH_STATUS, _time, _fishCurHp, String.valueOf(_anim), String.valueOf(_mode), String.valueOf(_goodUse), "", String.valueOf(pen), "0"));
		_anim = 0;
		if (_fishCurHp > (_fishMaxHp * 2))
		{
	...
	public synchronized void doDie(boolean win)
	{
		...
		if (win)
		{
			...
			if (fishingMonster != null)
			{
				...
				{
					_fisher.sendPacket(SystemMessageId.YOU_VE_WORN_THAT_FISH_OUT_IT_CAN_T_EVEN_PULL_THE_BOBBER_UNDER_THE_WATER);
	...
	protected void aiTask()
	{
		...
		finally
		{
			...
			final ExSendUIEventCustom efhr2 = new ExSendUIEventCustom(ExSendUIEventCustom.UPDATE_FISH_STATUS, _time, _fishCurHp, String.valueOf(_anim), String.valueOf(_mode), String.valueOf(_goodUse), "", "0", "0");
			if (_anim != 0)
			{
				_fisher.broadcastPacket(efhr);
				_fisher.broadcastPacket(efhr2);
			}
			else
			{
				_fisher.sendPacket(efhr);
				_fisher.sendPacket(efhr2);
			}
		}
	}
	...
	public void useReeling(int dmg, int pen)
	{
		...
		if (_mode == 1)
		{
			if (_deceptiveMode == 0)
			{
				...
				if (pen > 0)
				{
					sm = new SystemMessage(SystemMessageId.REELING_SUCCESSFUL_MASTERY_PENALTY_S1);
					sm.addInt(pen);
					_fisher.sendPacket(sm);
				}
				_goodUse = 1;
	...
	public void usePumping(int dmg, int pen)
	{
		...
		if (_mode == 0)
		{
			if (_deceptiveMode == 0)
			{
				...
				if (pen > 0)
				{
					sm = new SystemMessage(SystemMessageId.PUMPING_SUCCESSFUL_MASTERY_PENALTY_S1);
					sm.addInt(pen);
					_fisher.sendPacket(sm);
				}
				_goodUse = 1;
