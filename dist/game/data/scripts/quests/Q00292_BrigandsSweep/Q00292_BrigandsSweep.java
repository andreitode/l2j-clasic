		...
		switch (st.getState())
		{
			...
			case State.STARTED:
			{
				switch (npc.getId())
				{
					case SPIRON:
					{
						final long goblinNecklaces = getQuestItemsCount(player, GOBLIN_NECKLACE);
						final long goblinPendants = getQuestItemsCount(player, GOBLIN_PENDANT);
						final long goblinLordPendants = getQuestItemsCount(player, GOBLIN_LORD_PENDANT);
						final long suspiciousMemos = getQuestItemsCount(player, SUSPICIOUS_MEMO);
						final long countAll = goblinNecklaces + goblinPendants + goblinLordPendants;
						...
						else
						{
							...
							long reward = (12 * goblinNecklaces) + (36 * goblinPendants) + (33 * goblinLordPendants) + ((hasContract) ? 1120 : 0);
							if (countAll >= 10)
							{
								reward += 1000;
							}
