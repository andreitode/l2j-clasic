 ...
 */
package org.classiclude.gameserver.model.olympiad;
...
import org.classiclude.gameserver.network.serverpackets.ExOlympiadMatchResult;
import org.classiclude.gameserver.network.serverpackets.ExOlympiadUserInfo;
...
public abstract class OlympiadGameNormal extends AbstractOlympiadGame
{
	...
	protected void buffPlayers()
	{
		...
		Skill warriorBuff1 = SkillData.getInstance().getSkill(1204, 2);
		Skill warriorBuff2 = SkillData.getInstance().getSkill(1086, 1);
		Skill mageBuff1 = SkillData.getInstance().getSkill(1204, 2);
		...
		Consumer<Player> applyBuffs = player ->
		{
			if (player == null)
			{
				return;
			...
			if (Arrays.stream(warriorClasses).anyMatch(id -> id == classId))
			{
				if (warriorBuff1 != null)
				{
					warriorBuff1.applyEffects(player, player);
				}
				if (warriorBuff2 != null)
				{
					warriorBuff2.applyEffects(player, player);
				}
			}
			else if (Arrays.stream(mageClasses).anyMatch(id -> id == classId))
			{
				if (mageBuff1 != null)
				{
					mageBuff1.applyEffects(player, player);
				}
			}
		};
		
		applyBuffs.accept(_playerOne.getPlayer());
		applyBuffs.accept(_playerTwo.getPlayer());
	}
}
