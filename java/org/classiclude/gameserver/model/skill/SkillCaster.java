 ...
 */
package org.classiclude.gameserver.model.skill;
...
import org.classiclude.gameserver.network.serverpackets.MagicSkillCanceled;
import org.classiclude.gameserver.network.serverpackets.MagicSkillLaunched;
		...
		try
		{
			...
			for (WorldObject obj : targets)
			{
				...
				if (!Config.RAID_DISABLE_CURSE && creature.isRaid() && creature.giveRaidCurse() && (caster.getLevel() >= (creature.getLevel() + 9)) && (skill.isBad() || ((creature.getTarget() == caster) && creature.asAttackable().getAggroList().containsKey(caster))))
				{
					...
					caster.abortCast();
					caster.sendMessage("You have been cursed for attacking a Raid Boss above your level.");
					caster.sendPacket(ActionFailed.STATIC_PACKET);
					return;
				}
